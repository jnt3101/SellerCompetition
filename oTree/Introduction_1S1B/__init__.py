...
- Players are assigned to fixed matching groups of size 2
  (1 seller, 1 buyer)
...
class Player(BasePlayer):
    ...
    seller_comp_3 = models.IntegerField(
        ...
        choices=[
            ...
            [3, "Um den Preis, zu dem die Lotterie verkauft wurde."],
        ],
        widget=widgets.RadioSelect,
        blank=True,
    )
...
def creating_session(subsession: Subsession):
    ...
    At session creation, we assign:
        * fixed matching groups of size 2 (1 seller, 1 buyer)
    ...
    group_size_matching = 2
    group_type_value = "1S1B"

    if n_players % group_size_matching != 0:
        raise Exception(
            "Total number of participants must be a multiple of 2 "
            "(1 seller, 1 buyer per group). "
    ...
    # Assign matching group IDs: blocks of 2 participants
